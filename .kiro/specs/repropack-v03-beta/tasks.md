...
- [x] 1. Add new workspace dependencies and update crate Cargo.toml files
...
- [x] 2. Implement repropack-model new types and modified types
  - [x] 2.1 Add new types: `RedactionEntry`, `RedactionAction`, `DoctorReport`, `DoctorReadiness`, `RedactionSummary`
  ...
  - [x] 2.2 Add configuration types: `RepropackConfig`, `ProfileConfig`, `ResolvedConfig`
  ...
  - [x] 2.3 Add `extract_semver` helper function
  ...
  - [x] 2.4 Modify `ReplayReceipt` to add `env_excluded_keys` optional field
    - Add `env_excluded_keys: Option<Vec<String>>` with `#[serde(default, skip_serializing_if = "Option::is_none")]`
    - _Requirements: 3.2, 20.1_

  - [x] 2.5 Modify `PacketManifest` to add `redaction_report_path` optional field
    - Add `redaction_report_path: Option<String>` with `#[serde(default, skip_serializing_if = "Option::is_none")]`
    - _Requirements: 9.6, 9.9, 20.2_

- [x] 3. Checkpoint — model types
  - Ensure all tests pass, ask the user if questions arise.

- [x] 4. Implement repropack-git changes
  - [x] 4.1 Add path exclusion support to `capture_git_snapshot`
  ...
  - [x] 4.2 Add bundle creation fallback for root commits
...
- [x] 5. Implement repropack-capture changes
  - [x] 5.1 Update default packet naming
  ...
  - [x] 5.2 Add `CaptureOptions::from_config` constructor
    - Map `ResolvedConfig` fields to `CaptureOptions` fields
    - _Requirements: 14.2, 16.2_

- [x] 6. Implement repropack-replay changes
  - [x] 6.1 Pass exclusion paths to `capture_git_snapshot` calls
    - Pass `&[".repropack-replay"]` for both pre-run and post-run snapshot captures
    - _Requirements: 1.1, 1.2, 1.3, 19.1, 19.2, 19.3_

  - [x] 6.2 Implement env-excluded drift collapse
  ...
  - [x] 6.3 Implement tool version normalization
  ...
  - [x] 6.4 Add `--verbose` flag support to `ReplayOptions`
...
- [x] 7. Checkpoint — git, capture, and replay layers
  - Ensure all tests pass, ask the user if questions arise.

- [x] 8. Implement repropack-render new functions
  - [x] 8.1 Add `render_doctor_text` and `render_doctor_json`
  ...
  - [x] 8.2 Add `render_explain_output`
  ...
  - [x] 8.4 Update receipt renderer for env-excluded summary
