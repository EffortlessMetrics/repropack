...
- [ ] 1. Add workspace dependencies and update crate Cargo.toml files
...
- [ ] 2. Implement repropack-model new types and modified types
  - [ ] 2.1 Add new structs: `GitSnapshot`, `CaptureDelta`, `EnvClassification`, `IntegrityEntry`, `SizeCaps`
  ...
  - [ ] 2.2 Modify `GitState` to add `git_pre`, `git_post`, `capture_delta` optional fields
  ...
  - [ ] 2.3 Modify `ExecutionRecord` to add `stdout_sha256` and `stderr_sha256` optional fields
    - Add `stdout_sha256: Option<String>`, `stderr_sha256: Option<String>` with serde defaults
    - _Requirements: 6.1_

  - [ ] 2.4 Modify `ReplayReceipt` to add `env_classification` and `matched_outputs` optional fields
...
- [ ] 3. Implement schema validation module in repropack-model
  - [ ] 3.1 Create `validate` module with `validate_manifest` and `validate_receipt` functions
  ...
  - [ ] 3.2 Integrate schema validation into `PacketManifest::read_from_path` and `ReplayReceipt::read_from_path`
...
- [ ] 4. Update JSON Schema files for v0.2 fields
  - [ ] 4.1 Update `schema/manifest-v1.schema.json`
...
- [ ] 6. Implement repropack-pack integrity envelope and sha256_bytes
  ...
  - [ ] 6.3 Update `materialize` to call `verify_integrity` when `integrity.json` is present
...
- [ ] 7. Implement repropack-git snapshot and delta functions
  - [ ] 7.1 Add `capture_git_snapshot` function
  ...
  - [ ] 7.2 Add `compute_capture_delta` function
  ...
  - [ ]* 7.3 Write property test for capture delta correctness (Property 1)
...
- [ ] 9. Implement repropack-capture pre/post orchestration, evidence digests, size caps, and integrity envelope
  ...
  - [ ] 9.2 Compute and store stdout/stderr evidence digests
  ...
  - [ ] 9.3 Implement size caps in artifact collection
  ...
  - [ ] 9.4 Generate integrity envelope after staging
  ...
  - [ ]* 9.5 Write property test for per-file size cap (Property 10)
    - **Property 10: Per-file size cap truncates and records omission**
    - **Validates: Requirements 11.1, 11.2**

  - [ ]* 9.6 Write property test for total packet size cap (Property 11)
...
- [ ] 10. Implement repropack-replay minimal env baseline, env classification, evidence comparison, and delta drift
  - [ ] 10.1 Replace host env inheritance with minimal baseline
  ...
  - [ ] 10.2 Implement `--inherit-env` fallback
  ...
  - [ ] 10.3 Implement environment variable classification
  ...
  - [ ] 10.4 Implement stdout/stderr evidence digest comparison
  ...
  - [ ] 10.5 Implement output artifact digest comparison
  ...
  - [ ] 10.6 Implement same-exit-different-evidence detection
  ...
  - [ ] 10.7 Implement capture delta drift comparison
  ...
  - [ ]* 10.8 Write property test for replay env baseline (Property 2)
    - **Property 2: Replay environment baseline contains only declared variables**
    - **Validates: Requirements 4.1, 4.2, 4.3**

  - [ ]* 10.9 Write property test for env classification partition (Property 3)
    - **Property 3: Environment classification is a disjoint partition**
    - **Validates: Requirements 5.1, 5.2**

  - [ ]* 10.10 Write property test for evidence digest drift (Property 4)
    - **Property 4: Evidence digest mismatch produces drift**
    - **Validates: Requirements 6.3, 6.4, 7.2**

  - [ ]* 10.11 Write property test for matched_outputs conjunction (Property 5)
    - **Property 5: matched_outputs is the conjunction of all output comparisons**
    - **Validates: Requirements 7.4**

  - [ ]* 10.12 Write property test for matched = exit AND evidence (Property 6)
    - **Property 6: Receipt matched equals exit code match AND evidence match**
    - **Validates: Requirements 8.1, 8.2**

  - [ ]* 10.13 Write property test for capture delta drift comparison (Property 17)
    - **Property 17: Capture delta drift comparison**
    - **Validates: Requirements 18.2**

- [ ] 11. Checkpoint — capture and replay orchestration
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 12. Update repropack-render for new receipt fields
...
- [ ] 13. Update repropack-cli for new flags
  - [ ] 13.1 Add `--inherit-env` flag to replay subcommand
    - Wire to `ReplayOptions.inherit_env`
    - _Requirements: 4.5_

  - [ ] 13.2 Add `--max-file-size` and `--max-packet-size` flags to capture subcommand
    - Default values: 52428800 (50 MiB) and 524288000 (500 MiB)
    - Wire to `CaptureOptions.size_caps`
    - _Requirements: 11.1, 11.3_

- [ ] 14. Checkpoint — render and CLI
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 15. Property tests for repropack-model serde round-trips and schema conformance
  - [ ]* 15.1 Write property test for manifest serde round-trip (Property 12)
  ...
  - [ ]* 15.2 Write property test for receipt serde round-trip (Property 13)
  ...
  - [ ]* 15.3 Write property test for schema rejection of invalid JSON (Property 7)
...
- [ ] 16. Property tests for repropack-pack round-trip, determinism, integrity, and path traversal
  - [ ]* 16.1 Write property test for pack/unpack round-trip (Property 14)
...
- [ ] 18. CLI smoke tests
  ...
  - [ ]* 18.4 `repropack capture` with no command exits non-zero with error
    - _Requirements: 16.4_

- [ ] 19. Malformed packet rejection tests
